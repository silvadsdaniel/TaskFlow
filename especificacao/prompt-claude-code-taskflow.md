# Prompt de Implementação — Claude Code

Este documento tem três partes:

- **Parte A** — o que colar no Claude Code para iniciar
- **Parte B** — conteúdo do arquivo `CLAUDE.md` (regras permanentes do projeto)
- **Parte C** — conteúdo do arquivo `SPEC.md` (especificação funcional e técnica)

O motivo da separação: prompt longo colado no chat se perde ao longo da sessão. Regras em arquivo no repositório o Claude Code relê a cada tarefa. Crie os dois arquivos antes de pedir qualquer código.

---

## Parte A — Prompt de abertura

```
Vamos construir um app web de lista de tarefas pessoal. Já existem dois arquivos no repositório: CLAUDE.md com as regras do projeto e SPEC.md com a especificação completa. Leia os dois antes de escrever qualquer código.

Também vou anexar as telas do design. Siga-as com fidelidade — espaçamentos, tipografia e cores estão definidos lá e não devem ser reinterpretados.

Trabalhe em fases, na ordem do plano descrito no SPEC.md. Ao fim de cada fase:
1. rode o build e o lint e corrija tudo antes de continuar
2. me mostre o que ficou pronto e o que ficou pendente
3. pare e espere minha confirmação antes de iniciar a próxima fase

Não pule fases, não implemente adiantado o que está previsto para uma fase posterior, e não instale nenhuma dependência sem me perguntar antes.
